...
DISPLAY_COLS = (
    "DATA_NEG","SEGMENTO","CLIENTE","AGÊNCIA","CNPJ","VALOR_TARIFA","VALOR_AUTORIZADO",
    "VALOR_DA_LAR", "TIPO_CLIENTE", "AUTORIZAÇÃO", "PRAZO", "VENCIMENTO", "OBSERVAÇÃO",
    "ATUADO_SCT", "MOTIVO", "TARIFA", "STATUS_CLIENTE", "ATUAÇÃO"
    )
...
DEFAULT_FILTERABLE_COLS = ("DATA_NEG")
...
class LarConsultaView(QWidget):
    ...
    def _build(self):
        ...
        _setw("CLIENTE", 320)
        _setw("VALOR_TARIFA", 160)
        _setw("VALOR_AUTORIZADO", 160)
        _setw("OBSERVAÇÃO", 380)
        _setw("VALOR_AUTORIZADO", 160)
        _setw("VALOR_LAR", 180)
        _setw("AUTORIZAÇÃO", 160)
        _setw("VALOR_DA_LAR", 160)
        _setw("STATUS_CLIENTE", 160)
    ...
    def prefill(self, f : Dict[str,str], autorun : bool=False):
        ...
        seg = get_ci("SEGMENTO", "SEG", "Segmento")
        cli = get_ci("CLIENTE", "cli", "Cliente")
    ...
    def clear_filters(self):
        for w in (self.ed_cli, self.ed_cnpj, self.ed_seg):
            w.clear()
    ...
    def _repo_search(self, seg: str, cli: str, cnpj: str) -> List[dict]:
        ...
        if not hasattr(self.repo, "search_consulta"):
            raise AttributeError("Implante LarRepository.search_consulta(...)")
