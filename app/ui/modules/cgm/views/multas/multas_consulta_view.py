...
class MultasConsultaView(QWidget):
    ...
    def _build(self):
        ...
        self.table.setHorizontalHeaderLabels(["Cliente","CNPJ","Segmento","Tarifa","Qtde","Situação"])
    ...
    def _do_query(self):
        ...
        for r, (c, cnpjx, segx, tarifa, qtde, sit) in enumerate(rows):
            for cidx, val in enumerate((c, cnpjx, segx, tarifa, qtde, sit)):
                self.table.setItem(r, cidx, QTableWidgetItem(str(val)))
        self.table.setVisible(True)

    # mock
    def _mock_query(self, seg, cli, cnpj) -> List[Tuple[str,str,str,str,int,str]]:
        base = [
            ("ACME LTDA","12.345.678/0001-90","Corporate","665",1,"Ativo"),
            ("Beta SA",  "22.333.444/0001-66","PJ","662",2,"Revisar"),
        ]
        def ok(a,b): return (not a) or (a.lower() in b.lower())
        out=[]
        for c,cnpjx,segx,tar,qt,sit in base:
            if ok(cli,c) and ok(cnpj,cnpjx) and ok(seg,segx):
                out.append((c,cnpjx,segx,tar,qt,sit))
        return out
