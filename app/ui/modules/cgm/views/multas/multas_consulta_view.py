...
class MultasConsultaView(QWidget):
    ...
    def _build(self):
        ...
        filt.addWidget(self.ed_cnpj); filt.addWidget(bt); filt.addStretch()
        ...
        self.table.setHorizontalHeaderLabels(["Cliente","CNPJ","Segmento","AG","CC","Situação"])
    ...
    def _do_query(self):
        if not any([self.ed_seg.text().strip(), self.ed_cli.text().strip(), self.ed_cnpj.text().strip()]):
            ...
            return
        rows = [
            ("ACME LTDA","12.345.678/0001-90","Corporate","0012","123456","Em análise"),
        ]
