...
# quais colunas terão menu de filtro:
DEFAULT_FILTERABLE_COLS = ("DATA_NEG", "TARIFA")

def _btn(text: str, accent: bool = True, w: int =160, h: int=36):
    b = QPushButton(text)
...
class MultasConsultaView(QWidget):
    ...
    def _build(self):
        ...
        _setw("CLIENTE", 320); _setw("OBSERVAÇÃO", 380)
        _setw("VALOR_TARIFA", 120); _setw("VALOR_AUTORIZADO", 160)
        _setw("AUTORIZAÇÃO", 180); _setw("VENCIMENTO", 120)
    ...
    def prefill(self, f: Dict[str, str], autorun: bool = False):
        ...
        # se veio de "Visualizar", não limpe no próximo show
        self._clear_on_next_show = False
    ...
    def hideEvent(self, e):  # mantém estado ao minimizar
        try:
    ...
    def _repo_search(self, seg: str, cli: str, cnpj: str) -> List[dict]:
        ...
        if not hasattr(self.repo, "search_consulta"):
            raise AttributeError("Implemente MultasRepository.search_consulta(...)")
        return self.repo.search_consulta(segmento=seg, cliente=cli, cnpj=cnpj)

    def _do(self, force=None):
        # force=True quando veio de prefill(autorun=True)
        if force is None:
        ...
        if not any([seg, cli, cnpj]) and not force:
    ...
    def _confirm_delete(self):
        ...
        if not ids:
            QMessageBox.information(self, "Excluir", "Selecione ao menos um registro."); return
